...
export default async function productsBySubcategoryGet({
  id = '',
  page = 1,
  total = 9
}: ProductGetParams = {}) {
  try {
    ...
    const response = await fetch(url, {
      next: {
        revalidate: 0,
        tags: ['products-by-subcategory']
      }
    });

    if (!response.ok)
      throw new Error('Erro ao pegar os produtos por subcategoria.');

    const data = (await response.json()) as { products: ProductApi[] };

    return data;
  } catch (error) {
    console.log('erro no fetch dos produtos por subcategoria,', error);
  }
}
